...
- **AC-16 (qa) A가 움직이면 B가 본다.** Given 클라이언트 2개가 접속, When A만 30초간 전방 추력을 넣고 B는 아무 입력도 보내지 않는다, Then (a) **B가 받은 스냅샷 안의 A의 위치**가 단조적으로 변하고 총 이동 거리가 `max_speed_mps`와 시간으로 설명되는 범위 안이다, (b) **B의 함선의 최대 이탈 `max_i dist(p_i, p_0)` 가 `0.05 m` 이하다**(끝점 거리가 아니다 — 끝점은 왕복 이탈을 0으로 읽는다) — 기대값은 **정확히 0**이고 0.05 m 는 위치 양자화 LSB(1 mm)의 50배로 잡은 여유다(R14 판정). *근거: B 는 `ShipPhysicsState::at_rest` 로 스폰하므로 속도·각속도가 0이고, 보내는 입력은 추력 0·롤 0·목표 자세 = 현재 자세다. 가속이 없으므로 위치 정수는 상수여야 한다. **SC-64 의 2 m 를 빌려 오지 않는다** — 그 값은 "두 관측 경로(예측 vs 보간+렌더 평활화)의 차이" 예산이고(관측 상한 0.3151 m), 이 항목이 재는 것은 "가속이 0인 물체의 물리적 변위"다. 서로 다른 양이다.* **관측값을 반드시 적는다. 1 mm(LSB 1개)를 넘으면 통과하더라도 한 문장으로 설명한다** — 움직일 이유가 없는 것이 움직인 것이다. **그리고 이 절은 대조 없이는 뜻이 없다: 같은 창에서 A 의 순변위 ≥ 100 m 여야 한다.** *`(a)` 의 "이동 거리 > 0" 은 이 자리를 못 막는다 — 1 mm 로도 참이라 **아무도 움직이지 않은 세션**이 (a)와 (b)를 동시에 통과시킨다. 계약 SC-62 의 자명 통과 방어 (i)(ii)(iii)이 정본이다.*, (c) 같은 tick의 스냅샷에서 **A가 본 자기 위치와 B가 본 A의 위치가 양자화 정수로 완전히 같다**, (d) 검사한 스냅샷 쌍의 수를 리포트에 적는다. **이것이 "여러 클라이언트가 같은 월드를 본다"의 실증이다.**
