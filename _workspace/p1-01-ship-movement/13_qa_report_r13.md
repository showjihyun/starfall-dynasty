...
### 4.4 그 수정의 후속 — 컴파일 확인 (§4.5 에서 테스트까지 닫혔다)
...
### 7.2 그 뒤의 상태 — **출처 게이트가 이 슬라이스에서 처음 초록이 됐다**

§7.1 은 r13 판정 시점의 상태다(고쳐 쓰지 않는다 — `_workspace/` 는 감사 기록이다).
그 뒤 블록 8 작업으로 `block8_invariants.py` 가 생기고 계약이 그것을 지명해 상태가 바뀌었다.

| 시점 | `check_item_sources.py` (기본) |
|---|---|
| §7.1 (r13 판정) | `85 / 90` · 위반 0 · **미지명 5**(SC-10·28·30·32·74) · exit **3** |
| 도구만 생겼을 때 (중간) | **출처 위반 5** — 계약이 도구를 지명하지 않았다 · exit **1** |
| 계약이 지명한 뒤 (현재) | **`90 / 90` · 위반 0 · 미지명 0 · exit 0** |

**중간 상태가 더 빨개진 것은 게이트의 옳은 동작이다** — 게이트 자신이 *"번호의 존재만 보는
역방향 게이트는 이것을 통과시킨다 — 그래서 이 검사가 있다"* 고 적고 있다.
계약을 고친 것은 architect 다. **qa 는 계약을 고치지 않는다.**

**남은 빨간불 하나 — 만기가 지금 돌아왔다**: `--include-rust` 는 **exit 1 · 8건**
(`SC-14` + `scenario.rs` 의 `SC-19·20·21·24·25·26·67`). 계약 §7b 규칙 8 의 만기가
**"블록 8 실행 전"** 이고, `14_block8_procedure.md` 가 블록 8 을 실행 가능하게 만들었으므로
**그 만기가 이제 도래했다.** 라벨은 `tools/bots/src` 소유자의 것이다.
...
## §4.5-L — SC-56 (e) 계측 수정: **미검증 해제** (리더 선행 기재, 2026-09-25)

> **번호 정정 · 아래 `### 4.5`(qa, 이 문서 말미)가 이 절의 판본이다.** 나는 이 절을 `§4.5`
> 로 붙였고 qa 가 같은 번호로 자기 절을 썼다 — 같은 문서에 `4.5` 가 둘이 됐다. 원칙 5 대로
> 내 기재를 지우지 않고 번호만 `§4.5-L` 로 갈랐다.
>
> **qa 가 이 절의 문장 하나를 정정했고 그 정정이 옳다.** 나는 `PeriodicStatusLogTests` 12/12
> PASS 를 *"세션 종료 로그 줄이 길어진 것이 기존 문자열 단언을 깨지 않았다"* 의 증거로 적었는데,
> client 는 `PeriodicStatusLog` 를 건드리지 않았으므로 **그 단언들은 애초에 위험에 없었다.**
> 그 통과는 손대지 않은 구성 요소의 회귀 검사이고 **바뀐 줄을 실행하지 않는다.** 내 결론이
> 실제로 기대는 것은 `failed=0 · 개수 불변` 이다 — 그것은 아래 `### 4.5` 가 적은 대로다.
>
> 그리고 qa 가 덧붙인 것: **바뀐 줄 자체를 단언하는 테스트는 하나도 없다.** 근거는 컴파일과
> 사람이 읽은 것뿐이다. 내 절은 그 공백을 적지 않았다.
...
### 4.5 **미검증 해제 — 테스트가 실제로 돌았다** (커밋 `1652742`)

Editor(PID 19348)가 풀려 `unity test` 가 돌았다. §4.4 가 `미검증(환경, E10)` 으로 남겨 둔 상태를
해제한다.

```
unity test client --mode EditMode  →  total=323  passed=321  failed=0  skipped=2   exit 0
PeriodicStatusLogTests (--filter)  →  12/12 PASS
```

**§7a — 이 초록불이 무언가를 쟀는가**

- **진짜 증거는 `total=323 / failed=0` 이다** — EditMode 전체에서 회귀가 없다.
- **`PeriodicStatusLogTests.cs:86-88` 12/12 PASS 의 범위를 정확히 적는다.** 그 셋은
  `PeriodicStatusLog` 의 포맷을 단언하고, client 는 **그 파일을 건드리지 않았다.** 따라서 그 통과는
  **손대지 않은 구성 요소의 회귀 검사**이고, **바뀐 로그 줄을 실행하지 않는다.**
  §4.4 와 그때 client 에게 보낸 말에서 나는 이 통과를 *"종료 로그 줄이 길어진 것이 기존 단언을
  깨지 않았다"* 의 증거로 적었는데, **그 표현은 느슨했다** — 그 단언들은 애초에 위험에 없었다.
  그 문장이 실제로 기대는 것은 위의 `failed=0 · 개수 불변` 이다.
- **바뀐 줄 자체를 단언하는 테스트는 없다.** `ObserverSession.OnSessionEnded()` ·
  `GreyboxSession` 의 세션 종료 로그 줄 내용을 검사하는 EditMode 테스트가 **하나도 없다**
  (`grep OnSessionEnded|session-end render smoothing client/Assets/_Project/Tests/` → 0건).
  그 줄이 옳다는 근거는 **컴파일 + 사람이 읽은 것**뿐이다. **계약이 요구하지 않는 커버리지 공백**
  이므로 결함으로 올리지 않고 사실로만 적는다 — 다음에 그 줄의 형식을 바꾸는 사람은
  **자동으로 알려 주는 것이 아무것도 없다**는 뜻이다.
- **테스트 개수가 323 으로 불변이다.** 이 수정은 테스트를 추가하지 않았으므로 그래야 했고,
  **그 단언은 실행 전에 지정됐다**(개수가 늘었다면 "무엇이 추가됐는가" 를 먼저 물었어야 한다).
- `skipped=2` 는 기준선과 같다 — 이번 수정으로 건너뛴 것이 아니다.

**그래도 SC-56 (e) 의 verdict 는 이 리포트에서 내지 않는다.** 이 수정이 한 것은
**판정을 가능하게 만든 것**이고 판정한 것이 아니다 — §4.2 가 적은 대로 ②③④ 에는
**위치 평활화가 실제로 발동한 세션**이 필요하고 블록 7 은 자세 밴드만 밟았다.

**남는 것 하나**: `PeriodicStatusLog` 에 같은 축 분리 결함이 남아 있다(범위 밖, `03_client_impl.md`
R24 절에 명시됨). **판정에 걸리지 않는다** — 그 로그만 보고 같은 오독을 할 사람을 위한 기록이다.

### 발견 4 (게이트 자신의 결함, 실측으로 걸렸다) — `--include-rust` 가 **조용히 무시됐다**

`check_item_sources.py:377` 이 `if args.include_rust and not args.tools_dir:` 였다.
**`--tools-dir` 를 함께 주면 `--include-rust` 가 아무 일도 하지 않고, 경고도 없다.**
그리고 `:401` 의 "Rust 제외" 안내줄도 `and not args.tools_dir` 가 붙어 **사라졌다.**

**실측으로 걸렸다** — 같은 계약에 두 명령을 돌려 서로 다른 답을 받았다:

| 명령 | 전 | 후(고친 뒤) |
|---|---|---|
| `--tools-dir tests/e2e --include-rust` | **exit 0** · Rust 위반 행 **0** · 제외 안내 **없음** | **exit 1** · Rust 위반 행 **8** |
| `--tools-dir tests/e2e` | exit 0 · 제외 안내 **없음** | exit 0 · 제외 안내 **1줄** |
| `--include-rust` | exit 1 · 8건 | exit 1 · 8건 (불변) |

**두 인자를 같이 준 사람은 Rust 를 검사했다고 믿은 채 exit 0 을 받는다** — 게이트에서 가장 나쁜
실패 형태이고, 이 슬라이스가 네 번 데인 형태 그대로다. 끄는 것과 **가리는 것**은 다르다.
`tests/e2e/` 는 qa 소유이므로 고쳤다: 플래그는 `--tools-dir` 와 무관하게 항상 먹고, 제외 안내는
Rust 를 안 훑은 모든 실행에서 나온다. **selftest 24 케이스 exit 0 불변.**

**이것이 §7.1 에 남긴 흠**: r13 §7.1 을 나는 `--tools-dir tests/e2e` 로 돌렸으므로
**그 출력에 "Rust 제외 8건" 줄이 없었다.** exit 코드와 위반 수는 옳았지만 **범위가 불완전했다** —
§7.1 을 읽는 사람은 Rust 가 그 판정에 포함됐다고 읽을 수 있었다. 지금은 그 줄이 나온다.
