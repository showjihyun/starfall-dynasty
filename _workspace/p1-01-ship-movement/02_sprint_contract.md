...
| SC-06 | **기동 거부 8종이 전부 실패하고 로그에 파일·필드·기대값이 나온다** — 검사 8건: ① `hard_boundary_radius_m = 20000.1` ② 스폰 `points_m` 빈 배열 ③ `point_count ≠ len(points_m)` ④ `tick_hz / snapshot_hz`가 정수 아님 ⑤ `reconnect_resume_window_seconds > linger_seconds` ⑥ 스폰 지점이 하드 경계 밖 ⑦ `main_thrust_mps2 < lateral` 또는 `< reverse` ⑧ 함선 클래스 `id` 중복 | **⚠ R25 재판정 — R24 의 이 칸을 대체한다.** *R24 는 "serde 문장에 필드가 들어 있다" 는 **구현 doc 주석을 읽고** 판정했다. 실제 로그를 보니 **거짓이었다**(§7a).* **갈리는 기준은 검증자가 필드와 1:1 인가다**: `de_points_m` 은 `points_m` 하나만 쓰므로 메시지가 **필드를 이름으로 말한다**(② → 만족). `de_boundary_radius_m` 은 **세 필드**(`soft_boundary_radius_m`·`hard_boundary_radius_m`·`visual_radius_m`), `de_accel_mps2` 는 **일곱 필드**가 공유하므로 로그의 `de_…` 는 **필드가 아니라 검증자 이름**이다(`de_f64_range!` 의 `stringify!($fn_name)`) — ① 은 **필드를 특정하지 못한다.** `at line/column` 은 대신이 되지 못한다: 위치는 필드 지칭이 아니고 `soft`/`hard` 가 **한 줄 차이**라(`cradle.json:16,17`) 잘못 고치기 쉽다. **`de_boundary_radius_m` 은 필드처럼 보이는데 필드가 아니다 — 없는 것보다 나쁘다.** **단언(층별)**: **`rule=derived`** → `file=`·`field=`·`expected=`·`actual=`·`rule=` 다섯 토큰 전부. **`rule=schema`** → `file=`·`rule=schema` **＋ 필드가 특정된다**(메시지가 필드명을 말하거나 `field=` 토큰이 있다) **＋ 심은 값이 메시지에 있다**(거부 이유가 심은 그것인지 묶는다, §7b(1)). **기계 검사(사람 눈 대신)**: `deserialize_with` 값별 필드 수를 세어 **1 을 넘는 검증자가 그 경로의 로그를 낸다면 필드가 특정되지 않는다.** 지금 `de_accel_mps2` 7 · `de_boundary_radius_m` 3 · `de_positive_upto_100000` 3 · `de_accel_deg_s2` 3. **→ ① 은 FAIL 이다. 미룰 개선이 아니다**(R24 의 "(b) 미룸" 은 전제가 거짓이어서 철회한다 — 요구를 만족하지 않으면 미루는 것이 아니라 FAIL 이다). **구제(server, 둘 중 하나)**: ⓐ `serde_path_to_error` 로 JSON Pointer 를 채운다(새 의존 1개) ⓑ 공유 검증자를 **필드당 하나로 쪼갠다**(의존 없음, 중복 늘어남). 어느 쪽이든 위 기계 검사가 1 로 떨어져야 닫힌다. **음성 대조 (R24 확정, 유지)**: 멀쩡한 사본 + **죽은 `DATABASE_URL`** 로 돌려 데이터 로딩은 통과하고 다음 단계에서 죽는 것을 보인다 — 포트를 잡지 않아 기동 금지와 양립한다. **⚠ 전제는 `main.rs` 의 순서(data 검증 → DB 연결)이고, 그 순서가 바뀌면 이 대조는 아무것도 증명하지 않으면서 계속 통과한다.** 전제를 함께 확인한다 — **`STARFALL_DATA_DIR`**(server가 S2에서 추가 확정)로 임시 사본을 가리키고 한 파일만 어긴다 → **종료 코드 1** + 로그 `ERROR 기동 거부: 데이터 검증 실패 file=<절대경로> field=<JSON Pointer> expected=<제약> actual=<값> rule=<schema\ |derived>`. **①②는 `rule=schema`, ③~⑧은 `rule=derived`**. **`data/` 원본은 건드리지 않는다**(designer 소유). 디렉토리를 못 찾으면 **찾은 경로를 로그에 찍고 기동 거부**한다 | server | AC-2(b~g), 태스크 §D-12, I-38 | E5 |
...
| SC-10 | `SHIP_SPAWNED`의 위치가 `data/`의 어느 스폰 지점과 **정확히 일치**하고, **잔류 만료 후**(> `linger_seconds`) 같은 `actor_id`로 재접속하면 새 `SHIP_SPAWNED`의 위치가 **첫 번째와 같다**(난수 아님) | **`python tests/e2e/block8_invariants.py spawn --system cradle`**(블록 8. 입력은 PostgreSQL `domain_events` 의 `SHIP_SPAWNED` 와 `data/world/systems/cradle.json`). 스폰 위치(mm 정수)를 `data/`의 `points_m` × 1000과 대조. 두 번째 스폰까지 **30초 + 여유 대기**(게이트 G-j). *30초 안의 재접속은 스폰이 아니라 재개이므로 이 항목으로 재지 않는다 — 그렇게 재면 항진명제가 된다(AC-3 주석)*. 도구가 `tick` 차이 > `linger_seconds × tick_hz` 인 같은 `actor_id` 의 스폰 짝이 **하나도 없으면 `미검증(표본 없음)`** 을 낸다 — 대기를 건너뛰면 통과하지 않는다. **전반부(위치가 `points_m` 와 정확히 일치)는 점유가 낮은 실행에서만 잰다(21차 신설)**: `cradle.json` 의 `assignment_rule` 이 배정 지점이 점유돼 있으면 `radial_offset_step_m`(150 m)씩 **반경 방향으로 민다**. 스폰 지점은 12개이므로 **31봇 단계 A 에서 재면 밀린 스폰이 정상적으로 발생하고 정상 동작이 FAIL 로 인쇄된다.** 그래서 이 항목은 **봇 1대짜리 짧은 실행 두 번**(사이에 `linger_seconds` 초과 대기)으로 재고 단계 A 와 섞지 않는다. `spawns_not_on_a_point > 0` 이면 FAIL 로 닫기 전에 **그 실행의 동시 접속 수**를 증거에 적는다 | server | AC-3(c), ADR-0010 §4 | E2, E5 |
...
| SC-24 | (b) `SetShipControlPayload`의 필드 목록에 위치·속도·현재 자세가 **없다**(타입이 곧 증거) (c) 범위 밖 값은 **클램프되지 않고 거부**되며 **그 tick에 직전 입력이 이월되어 함선이 계속 정상 이동한다** (d) `aim_*`를 극단값으로 계속 보내도 한 tick 회전량이 `turn_rate_max_deg_s × dt`를 넘지 않는다 | `tools/bots` 의 `probe` 시나리오 `cheat-range`·`cheat-range-turn` — (b) 파일:라인 + 필드 목록 (c)(d) 봇 치트 케이스. (c)는 거부와 **동시에 이동이 끊기지 않음**을 스냅샷으로 | server | AC-5(b~d), I-33, I-26, S-4-2·3 | E5, E6 |
...
| SC-28 | 열려 있는 모든 세션이 `snapshot_interval_ticks`마다 **정확히 1건**을 받고, 연속 두 스냅샷의 envelope `tick` 차이가 그 값(=2)과 같다. **단 각 세션의 첫 스냅샷은 검사에서 제외한다** | **`python tests/e2e/block8_invariants.py snapshots --bot-out-dir <봇 --out 디렉터리>`**(블록 8 단계 A. `SC-28_verdict` 를 따로 낸다). 봇 스냅샷 CSV에서 tick 차이 분포. **차이가 2가 아닌 쌍의 수 = 0**(첫 스냅샷 제외), **검사한 쌍 수와 제외한 건수(세션당 1건)를 적는다** — `checked_pairs == 0` 이면 PASS 가 아니라 `미검증(표본 없음)` 이다. **기대값의 출처는 `summary.json` 의 `snapshot_interval_ticks`**(서버가 `SESSION_READY` 로 알려 준 값)이고 도구에 박아 두지 않는다 — 박으면 `snapshot_hz` 가 바뀐 뒤에도 옛 수로 조용히 초록이 난다. **판정은 CSV 재계산으로 하고 봇의 `interval_violations` 는 대조로만 싣는다. 둘이 어긋나면 FAIL** 이다(어느 쪽이 맞든 그 실행으로는 닫을 수 없다 — I-25). **제외하는 이유**: 스냅샷은 **전역 tick 기준**(`tick % interval == 0`)으로 발사되므로 중간에 들어온 세션의 첫 간격은 **0~1 tick**이다. 세션별 위상을 두면 31세션이 서로 다른 tick에 직렬화를 요구해 **모든 tick이 스냅샷 tick**이 되고 부하가 2배가 된다(server ack §1-⑤) | server | AC-7(a) | E6 |
...
| SC-30 | 각 수신자의 `controlled_ship_id`가 자기 함선이고 `ships` 안에 있다 | **`python tests/e2e/block8_invariants.py snapshots --bot-out-dir <봇 --out 디렉터리>`**(블록 8 단계 A. `SC-30_verdict` 를 따로 낸다 — SC-28 과 같은 명령이지만 verdict 는 항목별이다). 봇마다 `controlled_ship_id`가 자기 `SHIP_SPAWNED`의 `ship_id`와 같은지 + **그 관측자가 받은 모든 스냅샷의 `ships` 안에** 존재. `ticks_checked == 0` 이거나 `controlled_ship_id` 가 실린 관측자가 없으면 `미검증(표본 없음)`. 봇의 `controlled_ship_missing` 은 대조로만 싣는다 | server | AC-7(c) | E6 |
...
| SC-32 | 잔류 함선의 `presence`가 `LINGERING`이고, **디스폰된 다음 스냅샷부터 `ships`에서 사라진다** | **`python tests/e2e/block8_invariants.py resume-despawn --resume-json <F>`** — 그 `<F>` 는 **`cd tools/bots && cargo run --release -- resume --label bot-007 --observer bot-008 --gap 35 --listen 45 --out <F>`**(`tools/bots` 는 별도 워크스페이스다 — 루트에서 `-p` 로 부르지 않는다) 의 산출물이다(블록 8). 봇 CSV의 `presence` 열 + 디스폰 tick 전후 스냅샷 비교. **`bots run` 의 어떤 단계도 이 항목의 표본을 만들지 못한다(21차 신설)**: `run` 은 항상 `bot-000…bot-(N-1)` 로 붙으므로 단계 B(세션 회전)의 다음 주기가 **같은 라벨로 돌아와 잔류 창 안의 재개**가 되고 디스폰이 일어나지 않으며, 단계 A·C·D 는 아무도 중간에 나가지 않고 실행이 끝나면 **지켜볼 관측자가 남지 않는다.** 두 `run` 을 겹치면 같은 actor 충돌이다(§0.11 계열 — SC-88). **`resume` 만이 "한쪽은 나가 있고 다른 쪽은 계속 본다" 를 만든다.** **판정량은 재등장 건수다** — *"마지막 등장 이후 안 보인다"* 는 정의상 참이라 그대로 재면 항진명제다. 그 0 이 뜻을 가지려면 도구가 단언하는 셋이 전부 서야 한다: **(a)** `--gap > linger_seconds`(30 이하면 재개이고 디스폰이 아니다 — G-j), **(b)** 관측자가 그 함선의 `LINGERING` 을 실제로 봤다, **(c)** `observer_last_tick > ship_last_tick`(관측자가 **함선보다 오래 봤다** — 아니면 "사라졌다" 와 "관측자가 먼저 눈을 감았다" 를 구분할 수 없다. `--listen` 은 `linger_seconds + 10` 초 이상). 하나라도 안 서면 `미검증(표본 없음)` 이고 **PASS 가 아니다** | server | AC-7(e), I-40 | E6 |
...
| SC-57 | 정상 종료 후 잔류 창이 지나면 DB에 그 함선의 `SHIP_DESPAWNED{LINGER_EXPIRED}`가 있다 | **정상 종료 = `close_reason = CLIENT_CLOSED` 다**(R24) — `SERVER_SHUTDOWN` 은 SC-13 이 따로 본다. **잔류 창 경계는 등호다**: `SHIP_DESPAWNED.tick − SESSION_CLOSED.tick` **== `linger_ticks`**. *`>` 도 `>=` 도 쓰지 않는다 — 디스폰 스윕이 매 tick 돌므로 조건이 참이 되는 첫 tick 이 정확히 `linger_ticks` 이고(`sim/src/simulation.rs` 의 `>= linger_ticks`), **등호만이 이르게 지는 경우와 늦게 지는 경우를 모두 잡는다.** `>` 로 쓰면 정상 동작 40건이 전부 "창을 안 넘겼다" 로 인쇄된다(qa-block1 실측).* — SQL(ship_id 기준) | client, qa | AC-13(d) | E2, E3 |
...
| SC-66 | 치트 (a): 위치·현재 자세 필드 주입 → **전부 `MALFORMED_COMMAND`, 상태 변화 0**. **시도 횟수와 차단 횟수를 적는다** | `tools/bots` 의 `probe` 치트 시나리오 2종 `cheat-position`·`cheat-attitude` — 봇 치트 케이스 2종 × N회 | qa | AC-17(a) | E6 |
...
| SC-74 | DB 중단 구간에도 **스냅샷은 계속 흐른다**(이동은 DB에 의존하지 않는다) | **`python tests/e2e/block8_invariants.py db-outage --bot-out-dir <stage-d> --stats-before <F> --stats-after <F>`**(블록 8 D 단계). D 단계에서 중단 구간에 봇이 받은 스냅샷 수 > 0(**봇별로** 센다 — 하나라도 0 이면 FAIL), 끊긴 연결 0(`server_initiated_closes` · 봇 `errors`). **중단은 `docker compose pause postgres` → 30초 → `unpause` 다 — `stop` 도 `down` 도 아니다(21차 신설)**: `stop` 은 컨테이너를 내리고 재기동 시 초기화 경로를 타 검증 창이 흐려지며, `down -v` 는 `domain_events` 를 지운다(절대 금지). **`--stats-before` 는 멈추기 직전, `--stats-after` 는 재개 직후에 받는다** — 시점이 틀리면 창 밖을 잰다. **도구가 "DB 가 실제로 멈췄다" 를 먼저 단언한다(21차 신설, 필수)**: 두 stats 사이에 `persist_backlog` 가 늘었거나 `domain_events_persist_failed_total` 이 늘어야 하고, **둘 다 그대로면 `미검증(증거 요건)` 이고 PASS 가 아니다.** postgres 가 멀쩡한 채로 잰 *"스냅샷이 흘렀다"* 는 아무것도 뜻하지 않는다 — **§7a 가 막으려는 바로 그 형태이고 이 슬라이스가 네 번 데인 형태다.** `outage_actually_happened.asserted` 를 증거에 적는다 | qa | AC-18(f), I-31 | E2, E6 |
...
| **`block8_invariants.py`** (신규 — 하위 명령 `snapshots`·`spawn`·`resume-despawn`·`db-outage`·`selftest`) | SC-10 · SC-28 · SC-30 · SC-32 · SC-74 | 블록 8. **입력이 셋**(봇 `--out` 디렉터리 · PostgreSQL `domain_events`+`data/world/systems/` · 중단 창 전후 `/debug/stats`)이라 하위 명령으로 갈랐다. **이 다섯은 Unity 도 사람 손도 필요 없다** — 서버·docker·봇만으로 돈다(절차서 `14_block8_procedure.md`). `resume-despawn` 이 별도 하위 명령인 것은 **`bots run` 의 어떤 단계도 SC-32 의 표본을 만들지 못하기 때문**이다(아래 SC-32 행). **하위 명령 하나가 verdict 하나를 내고**(규칙 5), 겨냥한 조건이 안 일어났으면 `미검증`(exit 4)이다 — `snapshots` 만 세 verdict 를 내지만 **항목별로 각각** 낸다 |
...
>    **(21차 보충) 규칙 7 의 대상은 `verdict` 라벨이다. 증거가 관련 항목을 *가리키는* 표식(`관측용`·`참고`·`참조`)은 위반이 아니다.**
...
**⚠ 규칙 7 이 잡아서는 안 되는 것 (R23).** `tools/bots/src/scenario.rs` 의 probe 라벨 11건 중 하나가 *"**관측용**: `SET_SHIP_CONTROL` 정상 주기 전송"* 이었다. **이것은 R18 이 잡은 병이 아니다** — R18 은 *도구가 남의 번호로 **verdict 를 낸다***이고, 이쪽은 *로그가 관련 항목을 **가리킨다***이다. 뒤의 것은 **증거를 항목에 이어 주는 유용한 표식**이다.

**구분이 없으면 게이트가 "로그에서 SC 번호를 전부 빼라"는 압력을 만든다.** 그러면 라벨 불일치는 사라지지만 **증거에서 항목으로 가는 길도 같이 사라진다** — 이 슬라이스가 리포트·증거를 추적할 수 있었던 이유가 그 번호들이다. **고치는 것이 재는 것을 망가뜨리는 형태**이고, §7 이 임계 인상을 금지한 것과 같은 종류의 함정이다.

→ 도구가 `관측용`·`참고`·`참조` 가 문자열 안에 있는 라벨은 세지 않는다. **대조 둘을 같은 실행에 둔다**(규칙 6): 표식이 있으면 통과, **같은 파일의 표식 없는 라벨은 잡는다.** 이 보충으로 알려진 불일치가 **11 → 8** 로 줄었다 — **줄어든 3건은 고쳐서가 아니라 애초에 위반이 아니었기 때문이다. 그 구분을 출력이 하지 못하면 사람이 3건을 지우러 간다.**
...
| 사례 | 비워 둔 이유 | 실제 | 값 |
|---|---|---|---|
| **SC-51·52** | 층이 갈릴지 모른다 | **하나였다** | **읽고도 갈리지 않아 물었다** — 비용은 왕복 한 번 |
| **SC-55** | 근거가 둘로 나뉜다(**리포트 서술**) | **칸에 이미 답이 있었다** | **틀렸다 — 읽지 않고 물었다** |
| **블록 8 의 5건** | 절차서가 아직 없다 | 지명할 실행이 **존재하지 않는다** | **옳다** |

**가운데 칸이 이 표의 요점이다** — "비워 둔 이유"는 셋 다 그럴듯한데, **무엇을 읽고 그렇게 판단했는지**가 갈린다. SC-55 만 **행이 아니라 행에 대한 서술**을 읽었다.

> **"모르면 비워 둔다"의 전제는 먼저 읽는 것이다. 비워 두기가 싸다는 이유로 읽기를 건너뛰면 그것은 규율이 아니라 규율의 흉내다.**

**세 사례의 성격이 다르다는 것도 같이 읽어야 한다**: qa 의 SC-03 자동 분류와 architect 의 F-3 자동 분류는 **선을 밟을 뻔한** 것이고, **SC-55 는 밟은 것**이다. 앞의 둘은 멈춰서 규율이 됐고, 뒤의 하나는 **다음 사람이 읽고 되돌려서** 규율이 됐다.
...
**⚠ 자기 산출물은 자기가 검사자일 때 가장 약하게 검사된다 (R23, 두 사례의 합).**

이 라운드에 *"무엇을 보지 않았는가가 출력에 없다"* 는 같은 형태를 **여섯 번** 만났다 — 항목의 분모 · exit 코드 · §9 누수 · 도구 종류 · 제외 미기재 · Rust 미훑기. **그중 셋은 qa 가 그 규율을 쓴 뒤에 qa 가 어겼다**(§9 누수 · 도구 종류 · 제외 미기재).

> **규율을 쓰는 것과 자기 도구에 적용하는 것은 다른 일이다.** 만든 사람에게 자기 검사는 *"이미 규율을 통과한 것"* 으로 보인다.

**R22 후속의 문장과 같은 뿌리다**: *"문서가 만든 오독은 그 문서를 쓴 사람에게 가장 잘 통한다 — 자기 문장은 의도로 읽히기 때문이다."* 둘을 나란히 두면 하나의 규율이 된다 — **자기 문장은 의도로 읽히고, 자기 검사는 통과한 것으로 보인다.** 그래서 **규칙 3**(판정 기준을 만드는 사람도 자기 기준에 1번을 돌린다)이 **도구와 문서에도 걸린다.**
...
| 미뤄둠 | 만기 | 만기가 지나면 |
|---|---|---|
| ~~계약 §1 의 **36개 행이 판정 수단을 지명하지 않는다** — 출처 게이트가 `54 / 90` 을 찍고 exit 1 이다~~ → **닫혔다(21차, 2026-09-25)**. 20차가 29건 + 후속 5건을 쓰고, **21차가 마지막 5건(SC-10·28·30·32·74)을 `block8_invariants.py` 로 지명했다.** 이 다섯의 만기는 *"블록 8 절차서가 명령을 확정하는 시점"* 이었고 **`14_block8_procedure.md` 가 그 시점이었다** — 명령이 확정됐으므로 *"문서가 절차서를 앞질러 지어낸다"* 는 미룸의 전제가 사라졌다. 게이트: **`90 / 90` · 미지명 0 · 출처 위반 0 · exit 0**(이 슬라이스에서 출처 게이트가 처음 초록이 된 지점) | ~~블록 8 실행 전~~ **만기 도달 전에 닫았다** | — |
| *"판정을 바꾸면 그 판정을 쓰는 모든 대조가 무엇을 재는지 다시 본다"* 를 규칙 6 의 절로 넣을지 | **슬라이스 종료 전** | 넣지 않기로 판단했으면 그 판단을 §9 에 적는다 |
| **규칙 9 의 이행 조항에 분모가 없다** — *"바이너리로 판정하는 항목"* 목록이 없으면 조항이 있어도 **어디에 거는지 모르고, 그 상태는 조항이 없는 상태와 구분되지 않는다**(규칙 7 이 §3.2 빈 표에서 겪은 것). **분모 없는 조항을 두지 않는다** | **슬라이스 종료 전** | 리포트 요약에 비-통과로 올라온다 |
| **`SC-67` 이 (b)/(d) 두 성질을 한 항목에 담는다** — 범위 초과 거부와 `input_seq` 역행이고 방법 칸은 도구 하나를 지명한다. **지금 쪼개지 않는다**: 항목 수 **90** 이 이 슬라이스의 회계 단위이고 91 이 되면 **닫힌 리포트 전부의 분모가 낡는다**(리더 판정). **⚠ 이 만기는 다른 만기와 종류가 다르다 — 다른 만기는 *"할 일이 남았다"* 를 세지만 이것은 *"할 필요가 생겼는가"* 를 센다. 그래서 분모 1 은 빨간불이 아니라 초록이고, 만기가 지나도 아무것도 하지 않는 것이 옳은 결과일 수 있다**(architect). 적지 않으면 다음 사람이 *"만기 미이행"* 으로 읽는다 — `도구 없음(사람 관찰)` 을 빈칸과 구별해야 하는 것과 같은 이치다 | **`SC-67` 을 verdict 로 판정하는 도구가 2 이상이 되는 시점** | **분모 = `SC-67` 을 verdict 로 내는 도구 수. 실측 = 1** (`cargo test -p starfall-sim --locked ack_input_seq`). `scenario.rs` 에서 `SC-67` 을 든 팔은 `CheatSeq` 하나이고 **`Reference` 라 verdict 가 아니다**; `tests/e2e` 에 `SC-67` 을 `"item"` 라벨로 내는 도구는 **없다**. **1 이면 닫혀 있다 — 쪼갤 필요가 없다.** 재는 법: Q-1 의 팔 파서가 종류와 함께 세므로 **`--include-rust` 실행의 부산물**이다 |
| **`data/ships/*.json` 이 줄어도 SC-07 이 못 본다** — glob 0 매치만 오류로 세므로 *"함선 클래스 한 벌이 사라졌다"* 를 통과시킨다. **`== 3` 을 박지 않는다**: `validate_data_files.py` 의 docstring 이 *"designer 가 한 벌 더 추가하면 자동으로 검사 대상"* 을 **설계로** 적고 있고 계약 SC-07 의 "3파일" 은 **glob 3종**이라, 고정 수는 **designer 의 정상적인 추가를 FAIL 로 인쇄한다** — 지워진 `RUST_KNOWN_MISMATCHES = 8` 의 **반대편 판**이다(손으로 적은 수가 *관측*을 앞지르면 낡아서 거짓을 인쇄하고, *정책*을 앞지르면 정상 변경을 결함으로 인쇄한다. 문제는 수를 두는 것이 아니라 **그 수가 무엇에서 유도되는가**다). **지금은 닫혀 있다 — 이유가 둘이다.** ① `ship_classes = 1` 이라 *한 벌 사라짐* = *전부 사라짐* 이고 glob 0 매치로 잡힌다(qa-block1 의 음성 대조 B 가 확인). ② **참조 무결성 검사도 걸 대상이 없다** — `grep -rn "scout-s01\|ship_class" data/` 가 **자기 id 한 줄**만 낸다(qa-block1). **"몇 개인가" 를 세지 않고 "참조된 것이 다 있는가" 를 보면, 수를 박지 않고도 안 박은 것을 정당화하지 않고도 닫히는 자리였다**(리더가 준 세 번째 길). **상세와 둘째 위험은 qa-block1 의 절차서 §F-5** — 같은 만기가 둘을 연다. **둘째가 더 크다**: 서버가 함선 클래스를 `BTreeMap` **사전순 첫 번째**로 고르므로(`main.rs:346`) `scout-s01` 보다 앞서는 id 를 추가하면 **월드가 쓰는 함선이 조용히 바뀌고**, `simulation.rs:1375` 가 `"scout-s01"` 을 박아 둬서 **테스트와 런타임이 다른 클래스를 본다** — **삭제보다 이쪽이 크다. 정상 작업이 조용히 다른 결과를 낸다** | **함선 클래스가 둘 이상이 되는 시점** (날짜가 아니고 분모가 있다 — `data/ships/*.json` 개수) | 그 시점에 **기대 수를 어디에 두는가**를 정한다(`data/` 소유가 designer 이므로 계약 문제다). 만기가 지나면 리포트 요약에 **비-통과**로 올린다 |
| ~~출처 게이트가 Rust 도구를 사실상 못 본다~~ **→ 닫혔다 (R28 Q-1~Q-8, 만기 내)** — 게이트가 Rust 를 **팔 단위로** 읽고 **케이스 이름**(§1 백틱 span 안 토큰 ∩ `as_str()`)으로 잇는다. **§3.1 을 지명원으로 쓰지 않는다** — 그 표의 치트 7종 이름 중 5개가 소스에 없어 **손으로 유지하는 대응표**가 필요해지고, 그것은 지워진 `RUST_KNOWN_MISMATCHES = 8` 과 같은 부패다(architect R28). **항등식 둘을 판정 *전에* 본다** — 뒤에 두면 파서가 팔을 놓쳐도 **판정이 먼저 초록을 내고 항등식이 나중에 빨개진다.** **그 위험이 실제로 일어났다**: rust 의 `use ContractRef::{…}` 한 줄이 수식 적중을 **0/17** 로 만들었고 **위반도 0 이라 '위반 0 · exit 0' 이 인쇄될 수 있었다 — 팔을 하나도 보지 못한 채로.** **항등식 ①(변이==팔)이 그것을 잡았고, 이 라운드에서 장치가 사람보다 먼저 잡은 첫 사례다.** **Q-8**: 비수식 `Verdict(`/`Reference(` 는 받지 않고 **exit 4** 다 — 정규식을 넓히면 **나중에 들어올 다른 enum 의 같은 토큰**을 verdict 주장으로 읽는다 | ~~블록 8 실행 전~~ **충족** | **실행 확인(qa) — 다섯 수를 따로 찍는다**: ① 변이 17 == 팔 17 · ② 주석 아닌 SC- 줄 22 == 판정 팔 17 + 도움말 5 · ③ 미지명 verdict 팔 **0** · ④ `.rs` 출처 위반 **0** · ⑤ 유령 지명 **0** · **⑥ `ContractRef` 명시 팔 17 / 17**(표식 0 · 기본 0 · 비수식 0) → **exit 0**, `--include-rust` 기본값. **⑥ 이 여섯째 수로 붙은 이유(Q-9)**: 종류 판정의 대체 사슬(`ContractRef::` → 관측 표식 → 기본 verdict)에서 **중간 단계가 조용했다** — `참고:` 두 글자로 verdict 주장 하나가 면제되는데 **항등식 ① 은 팔 수를 세므로 성립하고** 출력에 아무 표시가 없었다(architect 실측). **① 은 "게이트가 팔을 봤는가" 를 재고 "무엇이 종류를 정했는가" 는 재지 않는다.** 사슬과 기본값은 그대로 두고 **수 하나를 더해 소리 나게** 만들었다 — **분모를 찍는 규율은 결과의 분모뿐 아니라 판정 근거의 분모에도 걸린다.** `--selftest` **34케이스**(Rust 12). `--no-rust` 는 **판정만** 끄고 **항등식은 찍는다**; `--tools-dir tools/bots/src` 는 **exit 4**(11개를 받아 0개를 검사했으므로 `위반 없음` 을 인쇄하지 않는다). 증거 `evidence/R28-rust-gate/` |

**규칙 9 — 소스는 부정을 증명할 수 있지만 긍정은 증명하지 못한다.** (architect R27 문안, qa 삽입)

> 소스를 읽어 *"그 출력은 나올 수 없다"* 는 확정할 수 있다(**부정**). *"그 출력이 나왔다"* 는
> 확정할 수 없다(**긍정**) — **그 소스로 빌드됐는지, 그 경로가 실제로 돌았는지가 별개의 사실**이기
> 때문이다. **긍정의 단언은 실행 증거로만 닫는다.** doc 주석 · 타입 · 형식 문자열 · 데이터 파일의
> 값은 **근거가 아니라 가설**이다.
>
> **이행(바이너리 판)**: 바이너리로 판정하는 항목의 증거에 **`git rev-parse HEAD` + `git status
> --porcelain` 의 공백 여부**를 함께 남긴다.

**근거 사례 넷 — 전부 2026-09-25 하루에 나왔다.** architect R24 가 `DataError::Schema` 의 **doc 주석**
(*"필드·기대값·실제값이 문장에 들어 있다"*)을 증거로 SC-06 ①② 를 통과시켰는데 **그 주석이 거짓이었다**
(로그는 필드가 아니라 검증자 함수 이름을 찍는다). **리더 3건(리더 본인이 사례로 올렸다)**: ① SC-72 폴백을
**데이터 파일의 값**만 보고 중계한 것(서버 코드가 그 값을 읽는 곳이 없었다) · ② `git log --since` 로
바이너리 동일성을 닫으려 한 것(**커밋된 변경만 본다 — 워킹트리는 사후에 알 수 없다**) ·
③ **날짜 없는 타임스탬프**로 *"안 돌고 있다"* 를 단언한 것.

**이행 조항이 요구하는 것은 *"공백이다"* 가 아니라 *"공백 여부를 함께 남긴다"* 이다.**
더러운 트리는 **그대로 적는다** — 숨기면 반증 가능성이 죽는다.

**규칙 1 을 이 규칙 자신에 돌린 결과 — 못 거르는 것 셋:**

- **(가) 이 규칙은 "실행 증거가 있다" 만 요구하고 그 실행이 *무엇을 했는지* 는 묻지 않는다.**
  로그 한 줄만 있으면 통과한다. **규칙 9 는 규칙 1 을 대체하지 않고 짝이다** — 9 가
  *"소스가 아니라 실행을 봐라"*, 1 이 *"그 실행이 겨냥한 조건을 만들었는지 함께 단언하라"*.
- **(나) `HEAD` + 청결이 있어도 빌드가 그 트리에서 나왔다는 증명은 아니다**(오래된 타깃 캐시 ·
  다른 머신). **기록이 주는 것은 증명이 아니라 반증 가능성이다.** 더 강한 형태가 필요하면
  **빌드 산출물 해시**를 함께 남긴다 — 라운드 4 의 `e84cd098c06fce26` 이 그 관례이고
  **빠져 있던 것은 청결 여부였다.**
- **(다) 부정 쪽도 무제한이 아니다** — *"나올 수 없다"* 는 **읽은 소스가 그 경로의 전부일 때만**
  성립한다. 매크로 · `cfg` · feature 가 있으면 읽은 것이 전부가 아니다. **SC-06 ① 에서 그 조건이
  충족된 이유**: 형식 문자열이 **한 곳**(`server/crates/contracts/src/data.rs` 의 `de_f64_range!`)이고
  그 파일에 **미커밋 변경이 없었다.**

**내 소스를 읽을 때는 규칙 3 주석으로 간다.** 규칙 9 는 **소스를 증거로 쓸 때**의 규칙이고
(오늘 사례 넷은 전부 *남의* 소스·문서를 읽고 긍정을 단언한 것이다), **자기 것을 검사할 때**는
규칙 3 의 범위 주석이 다룬다(*자기 문장은 의도로 읽히고, 자기 검사는 통과한 것으로 보인다 —
도구와 문서에도 걸린다*). **둘은 뿌리가 다르므로 합치지 않았고, 읽는 사람이 헤매지 않게
이 줄만 걸어 둔다**(qa11 제안).

**적용 시점 — 소급하지 않는다.** 소급하면 오늘 닫힌 항목이 다시 열리는데 **그 재실행은 제품 결함을
하나도 드러내지 않는다**(관측 *내용*이 아니라 *출처 기록*의 문제다). **규칙이 들어간 뒤 새로 만들어지는
바이너리 기반 증거부터** 적용하고, 이미 닫힌 항목은 그대로 두되 **어떤 이유로든 다시 도는 항목은
그 순간부터 새 형식을 따른다.**

**첫 적용 사례**: SC-06 의 S-R25 이후 재실행(2026-09-25, 절차서 `15_block1_procedure.md` §A-4d).
`server_boot.py` 의 `source_provenance()` 가 HEAD · `status --porcelain` 전문 · 트리 청결 여부 ·
`server_binary_sha256_16` 을 증거에 자동으로 싣는다. **그 실행의 트리는 청결하지 않았고
(S-R25 커밋 대기 중, 12건) 그 사실을 그대로 기록했다.**
...
| 날짜 | 변경 | 사유 |
|------|------|------|
| 2026-09-25 (23차, architect 판정 R28) | **§1 네 행에 봇 probe 케이스 이름을 적었다 — 출처 게이트가 Rust 도구를 읽을 수 있게 하는 규칙 8 만기 2단계.** `:185` SC-89 (g) 에 `probe --case tick-burst`, `:204` SC-24 에 `cheat-range-turn`(기존 `cheat-range` 옆), `:274` SC-66 에 `cheat-position`·`cheat-attitude`, `:276` SC-68 (g) 에 `pre-ready`. **`:210` SC-25 는 일부러 고치지 않았다** — 그 행은 probe 가 아니라 `bots run` 을 부르고 `Stage` 에는 `contract_item()` 이 없어 SC 라벨을 한 줄도 내지 않는다(실측). 그리고 적으면 `CheatFlood` 라벨의 `SC-25` 혼입에 **사후 근거가 생긴다.** **이을 키는 §3.1 표가 아니라 §1 행의 백틱 span 안 토큰이다** — §3.1 의 치트 7종 이름(`position-injection` 등) 중 **5개는 `tools/bots/src` 에 존재하지 않는다**(실측). 그것을 지명원으로 쓰면 손으로 유지하는 대응표가 필요하고, 그것은 qa11 이 지운 `RUST_KNOWN_MISMATCHES = 8` 과 같은 종류의 부패다. **실측으로 나온 것**: `tools/bots/src` 의 주석 아닌 `SC-` 줄 22 = `contract_item` 팔 17 + `main.rs` `USAGE` 5 — **`1 / 22` 가 뜻을 잃은 것은 수가 작아서가 아니라 22 가 판정 라벨과 도움말을 섞었기 때문**이다. 현행 게이트가 못 보던 **다섯 건이 드러났다**: 출처 위반 2(`cheat-position`·`cheat-attitude` → SC-66)와 미지명 3(`tick-burst`·`pre-ready`·`cheat-range-turn`). **다섯 건 전부 '도구는 맞고 계약이 이름을 안 적었다' 이고 '남의 항목을 판정했다' 는 한 건도 없다** — 그래서 **소급 재판정을 하지 않고 라벨·계약 정리로 닫는다**(규칙 9 에서 소급하지 않기로 한 것과 같은 논증). **되돌리는 조건(기계가 보는 형태)**: Q-1~Q-8 의 **첫 실제 실행**에서 `.rs` 출처 위반 목록에 **SC-66 두 건 말고 다른 번호가 한 건이라도 나오면**, 그 번호를 쓰는 항목의 PASS 를 다시 연다. `SC-67`(`cheat-range-turn`)은 이 판정으로 라벨에서 **제거되는 것**이므로 그 예외에 들지 않는다. **verdict 표식을 산문에서 타입으로 옮긴다**: `참고:`/`관측용:` 부분문자열 규칙은 설명 문장에 그 두 글자가 들어간 verdict 라벨을 조용히 면제하므로, `ContractRef::{Verdict, Reference}` 로 가른다 — **오타가 나면 컴파일러가 잡는다.** `.py` 쪽은 `"item"` 키가 같은 일을 하므로 **규칙이 갈리는 게 아니라 같은 원리의 두 구현**이다. **만기 종료 조건을 다섯 수로 적었다**(항등식 ① 변이==팔==17 · 항등식 ② 17+5==22 · 미지명 verdict 팔 0 · `.rs` 출처 위반 0 · 유령 지명 0 → exit 0). 미지명 verdict 팔은 **exit 1**(3 이 아니다 — 계약의 문서 공백과 도구의 거짓 주장은 방향이 반대다), 분류 불가는 **exit 4**. **판정 기준 무변경**(합격 조건·수치·단언 한 글자도 안 고쳤고 이미 있던 도구의 이름만 적었다. 다만 검증 방법 칸의 **문면**은 네 행에서 바뀌었다) · `contracts/` 무변경 · 새 임계값 0건 · **항목 수 90 불변** · 파일 줄 수 814 불변 | architect `01_architect_decisions.md` `## R28 판정`. **실행 대조 6건(architect, 스크래치 — `tests/e2e/` 무수정)**: A 현재 = 위반 2 · 미지명 3 → exit 1 · B 계약+rust 수정 사본 = **exit 0**(종료 조건이 도달 가능) · C 양성 = `pre-ready` 를 `SC-13` 으로 바꾸면 **위반 1** · D 접두 = `cheat-range` 지명이 `cheat-range-turn` 으로 **새지 않는다**(미지명 1로 드러남) · E 자기 무력화 = 팔 파싱이 깨지면 **exit 4**(판정 전에) · **F 한 팔에 문자열 둘 = exit 0, 못 걸렀다** — 그래서 팔당 리터럴 1개를 강제하고 2개 이상은 exit 4 로 설계에 넣었다. **또 하나 실측**: `SC-55` 가 `:258`(§1)과 `:570`(§7 논의 표) 두 곳에 있고 `CONTRACT_ROW` 는 둘 다 잡는다 — 절 추적이 §3.2 분기에만 걸려 있어 **§1 밖 행도 지명으로 읽힐 수 있다.** `:570` 에 도구 이름이 없어 오늘은 무해하지만 막혀 있지 않다(항목 수가 맞게 나오는 것도 집합 덕분인 우연). 구현은 rust(`tools/bots/src`)·qa(`tests/e2e/check_item_sources.py`) 소유 |
| 2026-09-25 (22차, architect 판정 R24→R25 · qa 이력 기재) | **SC-06 의 사례 ①(`hard_boundary_radius_m = 20000.1`)을 FAIL 로 확정하고, ②~⑧ 은 `미검증(바이너리 시점)` 으로 둔다.** **①과 ②~⑧ 이 갈리는 이유가 이 라운드의 어휘다(R26)**: **소스는 부정을 증명할 수 있지만 긍정은 증명하지 못한다.** ① 의 FAIL 은 **소스만으로 확정된다** — 형식 문자열이 한 곳(`crates/contracts/src/data.rs:36-57` 의 `de_f64_range!`)이고 그 파일에 미커밋 변경이 없으므로 **HEAD 소스로 빌드된 어떤 바이너리도 그 사례에서 필드를 특정할 수 없다.** 반면 ②~⑧ 의 PASS 는 소스만으로 확정되지 않는다 — *소스가 `points_m` 을 형식 문자열에 갖고 있다*는 것은 *그 소스로 빌드된 바이너리가 그 로그를 실제로 냈다*는 증명이 아니다. **R25 가 고친 것은 기준이고 그것이 관측을 유효하게 만들지 않는다.** 구제는 server 소유(S-R25)다. R24 는 SC-06 방법 칸의 자기모순(8건 모두에 `field= expected= actual=` 을 요구하면서 ①②를 `rule=schema` 로 지정)을 *"판정 기준 칸이 요구한 것은 토큰이 아니라 문장이다"* 로 풀었다가 **R25 에서 철회했다** — 근거로 쓴 것이 `DataError::Schema` 의 **doc 주석**(*"필드·기대값·실제값이 문장에 들어 있다"*)이었고 **그 주석이 거짓이었다.** **doc 주석은 주장이지 증거가 아니다.** 실제 로그는 필드명이 아니라 `de_f64_range!` 의 `stringify!($fn_name)` — **검증자 함수 이름**을 찍는다. **qa 실측**: `de_boundary_radius_m` 은 **세 필드**가 공유한다(`soft_boundary_radius_m` `crates/contracts/src/data.rs:446` · `hard_boundary_radius_m` `:449` · `visual_radius_m` `:566`) — `error=de_boundary_radius_m 는 …` 한 줄로는 **셋 중 어느 것이 위반했는지 알 수 없다.** `de_accel_mps2` 는 **일곱 필드**가 공유한다. **리더 검산: 공유 검증자 10개 · 1:1 19개.** `at line/column` 은 대신이 못 된다(`soft`/`hard` 가 `cradle.json:16,17` 로 한 줄 차이이고, `de_boundary_radius_m` 은 **필드처럼 보이는데 필드가 아니라서 엉뚱한 필드를 고치도록 유도한다** — **없는 것보다 나쁘다**). **요구를 낮추지 않았다**: 구제(ⓐ `serde_path_to_error` 또는 ⓑ 공유 검증자 분리)가 싸고 표준적이며, **싸게 고칠 수 있는 요구를 낮추는 것은 §7 의 임계 인상과 같은 종류**다. 그리고 *"거부됐다"* 와 *"그 필드 때문에 거부됐다"* 는 다르고 **후자가 이 항목의 값**이다. R24 의 *"(b) 미룸"* 도 철회됐다 — **요구를 만족하지 않으면 미루는 게 아니라 FAIL 이다.** **②는 우연이 아니라 `de_points_m` 이 1:1 이라서 만족한다 — 로그 여덟 줄을 사람이 읽으면 그 우연이 전체를 대표하고, R24 가 정확히 그렇게 틀렸다.** 그래서 눈 대신 **기계 검사**를 넣었다: *`deserialize_with` 값별 필드 수가 **1 을 넘는 검증자가 그 경로의 로그를 내면 필드가 특정되지 않는다*** — **전부 1 로 떨어지는 것이 구제의 종료 조건**이고 `reject` 출력의 `validator_field_multiplicity.shared_count` 가 그 수다. **SC-57 방법 칸**: 정상 종료 = `close_reason=CLIENT_CLOSED`(SC-13 의 `SERVER_SHUTDOWN` 과 갈려야 한다), 경계는 **등호** `SHIP_DESPAWNED.tick − SESSION_CLOSED.tick == linger_ticks`(`>=` 는 **늦게 지는 경우를 통과시킨다**). `docs/adr/0011` §6 표가 잔류 창의 정본인데 **경계 방향이 없었다.** **SC-06 음성 대조 기법**(멀쩡한 `data/` 사본 + 죽은 `DATABASE_URL`)을 **전제 확인과 함께** 남겼다 — `main.rs` 의 data → DB 순서가 바뀌면 **아무것도 증명하지 않으면서 계속 통과한다.** **그리고 판정 도구의 판정량을 원인에서 결과로 옮겼다**: 처음에는 *"공유 검증자를 쓰는가"* 를 쟀는데, **공유는 필드가 특정되지 않는 원인이었지 그 자체가 위반이 아니다.** 구제 ⓐ(`serde_path_to_error`)는 **공유를 그대로 두고 JSON 경로를 앞에 붙여 필드를 특정한다** — 원인을 재는 검사는 그 고쳐진 로그를 **잘못된 이유로 FAIL 로 돌린다**(리더 지적). 이제 ① 로그가 **사례가 심은 필드를 지칭하는가**를 판정하고, ② 지칭하지 못하면 다중도를 **설명**으로 싣는다. **종료 조건도 `shared_count == 0` 이 아니라 schema 사례 전부가 `schema_names_injected_field = true` 가 되는 것**이다(ⓐ 에서는 공유가 0 이 되지 않는다). 자명 통과 두 자리를 함께 막았다 — **(가)** 검증자 이름이 필드명을 품은 경우(`de_points_m` ⊃ `points_m`)를 걸러내려 `de_*` 토큰을 먼저 지우고 찾는다, **(나)** *어떤 필드명이든 있으면 통과*로 두면 **엉뚱한 필드가 찍혀도 통과한다** — `soft` 를 위반시켰는데 `hard` 가 찍히는 것은 `cradle.json:16,17` **한 줄 차이**라 실제로 일어나고 architect 가 *"없는 것보다 나쁜 정보"* 라고 한 그 상황이므로, **심은 필드와 로그가 지칭하는 필드가 같은지**를 단언한다. `contracts/` 무변경 · 판정 기준 칸 무변경 · 새 임계값 0건 · 항목 수 90 불변 | architect `01_architect_decisions.md` `## R24 판정` · `## R25 재판정` · `### R25 후속 (R26)`. **qa 실행 확인**(절차서 `15_block1_procedure.md`, 증거 `evidence/R19-block1/`): `server_boot.py selftest` **16/16 exit 0**(`judge_case` 9 · schema 필드 특정 2 · `judge_negative_control` 5 — 음성에 *다른 필드로 거부* · *rule 이 다르다* · *심은 값이 로그에 없다* · *`file=` 토큰 없음* · *`file=` 는 있는데 다른 파일* · *세 필드 공유 검증자* · *data→DB 순서 뒤집힘* 포함); `reject` **새 빌드 2026-09-25T21:50:52 로 재실행 → 7/8 exit 1**, 음성 대조 PASS(`premise_data_before_db.asserted` true); **두 실행 대조 — 09-23 빌드와 새 빌드의 거부 로그가 8/8 동일**(`sc06-two-run-diff.txt`, 리더의 *"그 바이너리가 HEAD 소스와 다를 수 있다"* 를 **논증이 아니라 실행으로** 닫았다); `rejudge` 로 같은 로그를 R25 기준에 다시 걸어 **① PASS→FAIL 전환 1건**(`sc06-rejudged-r25.json`); SC-57 등호 음성 대조 — 간격 601 합성 입력(`VALUES` CTE, **DB 에 쓰지 않는다**)에서 **등호 1건 검출 / 옛 `>=` 0건**(`sc57-boundary-equality.txt`), 실데이터 40/40 == 600 위반 0; `check_item_sources.py --contract` **90/90 · 위반 0 · exit 0**. **S-R25 착수 시점이 이 판정 직후였다**(qa 실측: `Cargo.toml` 21:51:39 · server 빌드 21:52:06 · `bins/game-server/src/data.rs` 21:54:48, qa 의 빌드는 21:50:52) — **증거에 `server_binary_mtime` 을 박아 둔 덕분에 내 관측이 내 빌드의 것임이 사후에 확인됐다.** 그 수정이 `read_and_parse` 를 고쳐 **모든 schema 사례의 로그 형식이 바뀌므로 ②~⑧ 도 다시 돈다**(derived 사례는 안 바뀌었을 테지만 **안 바뀌었음을 재는 것도 관측이다**) |
| 2026-09-25 (21차, architect) | **블록 8 의 마지막 미지명 5건을 지명했다 — 출처 게이트가 이 슬라이스에서 처음 exit 0 이 됐다**(architect `## R24 판정`). **§3.2 에 `block8_invariants.py` 한 행**(하위 명령 `snapshots`·`spawn`·`resume-despawn`·`db-outage`·`selftest` → SC-10·28·30·32·74)과 **§1 다섯 행의 방법 칸**에 하위 명령을 적었다. qa 가 도구를 내자 게이트가 **exit 3(미지명 5) → exit 1(출처 위반 5)** 로 **더 빨개졌고, 그것이 옳은 동작이다** — 도구가 생겼는데 계약이 그 도구를 지명하지 않은 상태였다. **R20 이 두 빨간불의 exit 코드를 가른 것이 여기서 값을 냈다**: 코드가 3 → 1 로 바뀐 것 자체가 *종류가 다른 일이 생겼다* 는 신호였고, 섞여 있었다면 상시 빨간불 안에서 안 보였다. **qa 가 실행으로 찾은 셋을 계약이 말하지 않고 있었고 세 자리 다 고쳤다**(도구가 옳고 계약이 침묵한 것이지, 도구가 계약과 다른 것이 아니다): **① SC-32** — `bots run` 의 **어떤 단계도** 표본을 못 만든다(`run` 은 늘 `bot-000…` 이라 단계 B 의 다음 주기가 같은 라벨의 **잔류 창 안 재개**가 되고, A·C·D 는 아무도 안 나가며, 끝나면 **지켜볼 관측자가 없다**. 두 `run` 을 겹치면 SC-88 충돌). **`bots resume --gap 35 --listen 45` 경로를 명령까지 적고** 단언 셋(`gap > linger` · `LINGERING` 실관측 · **`observer_last_tick > ship_last_tick`**)과 **판정량 = 재등장 건수**를 넣었다 — *"마지막 등장 이후 안 보인다"* 는 항진명제다. **② SC-10 전반부** — 스폰 지점 12개에 `assignment_rule` 이 점유 시 150 m 씩 밀므로 **31봇 단계 A 에서 재면 정상 동작이 FAIL 로 인쇄된다.** 봇 1대 짧은 실행 두 번으로 재고 단계 A 와 섞지 않는다. **③ SC-74** — 옛 방법 칸(*"스냅샷 수 > 0, 끊긴 연결 0"*)은 **postgres 가 멀쩡해도 전부 참이다.** `--stats-before/--after` 필수와 **`persist_backlog`·`domain_events_persist_failed_total` 증가 단언**(없으면 `미검증(증거 요건)`), 그리고 **`pause`/`unpause` ≠ `stop` ≠ `down -v`** 구분을 계약에 넣었다 — 절차서에만 있었고 **절차서는 계약이 아니다.** **§7b 만기 표 첫 행(§1 미지명)을 닫았다** — 만기(`블록 8 실행 전`) 도달 **전에** 닫혔고, 미룸의 전제(*"문서가 절차서를 앞질러 지어낸다"*)는 `14_block8_procedure.md` 가 명령을 확정하면서 사라졌다. **SC-71 은 이 다섯을 막지 않는다**(qa 판단 확인): 봉쇄는 `gap > 20 %` **조건부**인데 실측이 아직 없고, 다섯의 판정 입력에 **ADR-0011 §2 의 수가 하나도 안 들어간다.** **다만 커플링 하나를 새로 적었다 — SC-71 이 아니라 SC-72 를 지난다**: 세션당 실측이 192 KiB/s 를 넘으면 designer 폴백이 `snapshot_hz` 10 → 5 를 당기고 **`snapshot_interval_ticks` 가 2 → 4 가 되어 그 전 단계 A 판정이 무효가 된다.** **조용한 초록은 안 난다** — SC-28 의 기대값을 `summary.json` 에서 읽게 계약에 명시했으므로(도구에 2 를 박았다면 바뀐 뒤에도 옛 수로 초록이었다); 남는 것은 **순서**뿐이고 그 한 줄은 절차서(qa 소유)에 넘겼다. **판정 기준 칸 무변경 · SC 행 90 유지 · 다섯 행 전부 7칸 유지 · `contracts/` 변경 없음 · 새 임계값 0건** | architect `01_architect_decisions.md` `## R24 판정`, qa `14_block8_procedure.md` §B·§A-0·§D·§G·§H 와 `tests/e2e/block8_invariants.py`. **실행 확인(architect)**: `check_item_sources.py --contract` 적용 전 → **`85 / 90` · 미지명 5 · 출처 위반 5 · exit 1**, 적용 후 → **`90 / 90` · 미지명 0 · 출처 위반 0 · exit 0**(이 슬라이스 최초의 초록); `--selftest` → **24케이스 PASS**; `check_contract_items.py --contract … --report 13_qa_report_r13.md` → **계약 항목 90 파싱 · 판정 행 5 · 위반 0 · exit 0**; `git diff --numstat` → **+8 / −7**. **읽어서 추론한 것(돌리지 않았다)**: SC-71 무관은 `block8_invariants.py` 소스와 ADR-0011 §2 를 **읽어** 확인했고 `bandwidth.py` 는 **실행하지 않았다**(31 연결 실측이 없어 돌릴 수 있는 상태가 아니다); `snapshot_hz` 폴백 커플링은 `data/movement/sync-tuning.json:10` 을 읽어 유도했고 **5 로 바꿔 돌려 보지 않았다**; `block8_invariants.py selftest` **20 케이스는 qa 보고를 인용한 것이고 내가 돌리지 않았다** |
