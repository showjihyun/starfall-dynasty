...
**짝이 되는 사례: SC-55.** 같은 라운드에 같은 사람이 같은 이유로 비워 둔 항목이 하나 더 있었는데, **이쪽은 실제로 갈려 있었다** — 리포트가 *"근거가 둘로 나뉘고 ② 쪽은 CL-1 대기"* 라고 적고 있고, qa 가 확인하니 **② 실S6 각속도 대조는 실행된 적이 없다**(`ReconciliationTests.cs` 231~371 구간에 `AngularVelocity` **0건**). **채웠다면 틀린 칸이 남았다.**

| | 비워 둔 이유 | 실제 | 비워 두기의 값 |
|---|---|---|---|
| **SC-51·52** | 층이 갈릴지 모른다 | **하나였다** | 비용이 **왕복 한 번**뿐이었다 |
| **SC-55** | 근거가 둘로 나뉜다 | **정말 갈려 있고 한쪽이 미실행** | **틀린 칸을 막았다** |

**둘을 같이 두면 읽히는 것**: *비워 두기는 대개 싸고, 가끔 결정적이다.* 하나만 두면 어느 쪽이든 절반만 보인다.
