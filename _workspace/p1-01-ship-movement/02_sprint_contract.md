...
| **출처 게이트가 Rust 도구를 사실상 못 본다** — 라벨 강등(커밋 `443ab3c`)으로 **p0-02 번호 10줄은 `참고:` 표식으로 처분됐다.** 그러나 **게이트가 볼 수 있는 줄이 `1 / 22`** 다(`RUST_LABEL` 이 한 줄짜리 화살표 라벨만 잡고, 실제 구조는 `Self::X => {` ⏎ `"…"` ⏎ `}` 다). **그래서 `--include-rust` 의 exit 0 은 "라벨이 옳다"가 아니라 "정규식이 보는 줄에 없다"는 뜻이다.** 게다가 `PY_NAME` 이 `.py` 만 잡아 **`.rs` 는 옳은 라벨이어도 지명될 수 없고**, 계약이 봇 도구를 **산문으로** 부르므로(`tools/bots` 의 `probe` 시나리오 …) `scenario.rs` 라는 문자열이 방법 칸에 없다 — **정규식 문제가 아니라 게이트 설계 문제다.** 게이트가 매 실행에 이 분수를 찍는다 | **블록 8 실행 전** | **닫히지 않았다.** 1단계(라벨 강등)만 끝났고 **2단계(§3.1 지명을 게이트가 읽게 만들기)가 남았다.** 열쇠는 파일명이 아니라 **probe 케이스 이름**(`cheat-range` 등, `scenario.rs` 가 enum 으로 들고 있다)으로 잇는 것이고 **설계는 architect 소유다.** 만기가 지나면 리포트 요약에 **비-통과**로 올린다 |
