...
| **출처 게이트가 Rust 도구를 안 본다** — `tools/bots/src/scenario.rs` 의 probe 설명 문자열이 **출력에 찍히는 라벨**인데 **p0-02 번호 11개**(SC-14·19·20·21·24·25·26·61·62·67)를 쓰고 있다. `--include-rust` 로 훑으면 보인다(exit 1). **기본값은 꺼 둔다** — 지금 켜면 게이트가 상시 exit 1 이 되어 **새 파이썬 위반을 가린다**(F-1 이 막으려던 형태) | **블록 8 실행 전** | 라벨을 고치고 **§3.1(봇 하네스)의 지명을 게이트가 읽게** 만든 뒤 `--include-rust` 를 기본값으로 올린다. 만기가 지나면 리포트 요약에 **비-통과**로 올린다 |
