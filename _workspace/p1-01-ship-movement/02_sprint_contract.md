...
**비워 두기가 실제로 작동한 사례 (R21, SC-51·52).** 규칙 7 주석의 기본값은 겁주는 말이 아니라 절차다. architect 가 SC-51·52 를 *"층이 갈릴지 모른다"* 는 이유로 **비워 두었고**, qa 가 리포트에서 **출처를 확정해**(`unity test` EditMode 의 한 테스트가 두 항목을 같이 낸다 — 그 로그 줄이 두 수를 그대로 싣는다) 채웠다. **결과적으로 층은 하나였지만, 하나임을 *확인하고* 쓴 것과 하나일 것이라 *짐작하고* 쓴 것은 결과가 같아도 다른 일이다.** 둘이었다면 틀린 칸이 남았다.
